...
from src.groqapi import GroqTimestampGenerator
...
class AIVideoSummarizer:
    ...
    def get_youtube_info(self):
        ...
        if os.getenv("GROQ_API_KEY"):
            self.model_env_checker.append("Groq")
        if os.getenv("GROQ_API_KEY_"):
            self.model_env_checker.append("Groq Chunk")
    ...
    def generate_summary(self):
        if st.button(":rainbow[**Get Summary**]"):
            ...
            if self.model_name == "Gemini":
                ...
                self.summary = Model.groq(transcript=self.video_transcript, prompt=Prompt.prompt1())
            elif self.model_name == "Groq Chunked":
                self.summary = Model.groq_chunked(transcript=self.transcript, prompt=Prompt.prompt1())
            st.markdown("## Summary:")
    ...
    def generate_time_stamps(self):
        if st.button(":rainbow[**Get Timestamps**]"):
            ...
            if self.model_name == "Gemini":
                ...
                self.time_stamps = Model.groq(self.video_transcript_time, Prompt.prompt1(ID='timestamp'), extra=youtube_url_full)
            elif self.model_name == "Groq Chunked":
                # self.time_stamps = GroqTimestampGenerator.groq_chunked(
                #     self.video_transcript_time,
                #       Prompt.prompt1(ID='timestamp'),
                #         extra=youtube_url_full)
                self.time_stamps = GroqTimestampGenerator.generate_timestamps(self.transcript , prompt_template=Prompt.prompt1(ID='timestamp'), video_url="")
    ...
    def generate_transcript(self):
        if st.button("Get Transcript"):
            self.video_transcript = GetVideo.transcript_time(self.youtube_url)
            self.transcript = GetVideo.reformat_transcript(self.video_transcript)
